...
class LstmClasifier(nn.Module):
    def __init__(self,input_size,hidden_size,n_classes,n_layer) -> None:
        super(LstmClasifier,self).__init__()

        self.n_layers = n_layer
        self.hidden_size = hidden_size

        self.lstm = nn.LSTM(input_size,hidden_size,n_layer,batch_first=True) # expect an input of [batch,sequence,input_size] 
    ...
    def forward(self,x):
        ...
        cs = torch.zeros(self.n_layers,x.shape[0],self.hidden_size) # [1,28,128] [layer,seq,hidden]
        out,_ = self.lstm(x,(h0,cs)) # an out with shape of [100,28,128]
    ...
    for epoch in range(epochs):
        for i,(feature,labels) in enumerate(train_loader):
            ...
            if i % loss_step  == 0:
                ...
                loss_counter = 0
    gr.plotLoss(all_losses,'LSTM loss',epochs)
